function [imgn,newret] = PerspectiveTransform(img,ret)
% clear ;
% close all;
% clc;
% 
...
% img(r) =255;
[M N] = size(img);
%  dot=ginput();       %取四个点，依次是左上，右上，左下，右下,这里我取的是书的四个
% [M N] = size(img);
% 
dot=fliplr(ret);       %取四个点，依次是左上，右上，左下，右下,这里我取的是书的四个角
% tmp = dot(1:2,:);
% dot(1:2,:) = dot(3:4,:);
% dot(3:4,:) =tmp;
...
mY=[dot(1,1) dot(1,1) dot(1,1)+w dot(1,1)+w];     
mX=[dot(1,2) dot(1,2)+h dot(1,2) dot(1,2)+h];
newret = [  mX' mY'];
% newret([2,3],:)=newret([3,2],:);
